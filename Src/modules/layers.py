...
class MaskAttention(torch.nn.Module):
    ...
    def forward(self, inputs, mask=None):
        scores = self.attention_layer(inputs).view(-1, inputs.size(1))  # (batch_size, 3)
        if mask is not None:
            scores = scores.masked_fill(mask == 0, float("-inf"))
        
        # 放大后两个特征的分数（w2, w3），仅当 inputs 是 aggregator 的输入（size(1) == 3）
        if inputs.size(1) == 3:
            amplification_factor = 1.5  # 可调，例如 1.5, 2.0
            scores[:, 1] *= amplification_factor  # 放大 w2 (reweight_expert_2)
            scores[:, 2] *= amplification_factor  # 放大 w3 (reweight_expert_3)
        
        scores = torch.softmax(scores, dim=-1).unsqueeze(1)  # (batch_size, 1, 3)
        outputs = torch.matmul(scores, inputs).squeeze(1)  # (batch_size, emb_dim)
